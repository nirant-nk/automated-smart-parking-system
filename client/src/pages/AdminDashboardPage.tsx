import { useState } from "react";
import Layout from "../components/common/Layout";
import ManageParking from "../components/admin/ManageParking";
import ManageRequests from "../components/admin/ManageRequests";
import ProtectedRoute from "../router/ProtectedRoute";

export default function AdminDashboardPage() {
  const [tab, setTab] = useState<"parking" | "requests">("parking");

  return (
    <ProtectedRoute roles={["admin", "owner"]}>
      <Layout>
        <div className="p-4">
          <div className="max-w-7xl mx-auto">
            <div className="bg-white bg-opacity-10 backdrop-blur-lg rounded-2xl p-6 mb-6 border border-white border-opacity-20">
              <div className="flex items-center justify-between">
                <h1 className="text-3xl font-bold text-white">Admin Dashboard</h1>
                <div className="bg-white bg-opacity-10 rounded-lg p-1 flex">
                  <button onClick={() => setTab("parking")} className={`px-4 py-2 rounded-md ${tab === "parking" ? "bg-white bg-opacity-20 text-white" : "text-gray-200"}`}>Manage Parking</button>
                  <button onClick={() => setTab("requests")} className={`px-4 py-2 rounded-md ${tab === "requests" ? "bg-white bg-opacity-20 text-white" : "text-gray-200"}`}>Manage Requests</button>
                </div>
              </div>
            </div>

            {tab === "parking" ? <ManageParking /> : <ManageRequests />}
          </div>
        </div>
      </Layout>
    </ProtectedRoute>
  );
}
